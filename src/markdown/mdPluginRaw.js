...
function simpleMath(state, silent) {
    ...
    var endMarker = "$";
    var afterStartMarker = state.src.charCodeAt(++startMathPos); // eslint-disable-line
    ...
    if (token.type === "equation_math_not_number") {
        ...
        equationNode =
            token.type === "equation_math"
                ? "<span class='equation-number' " + (tagId ? "id=\"" + tagId + "\"" : "") + ">(" + ++count + ")</span>"
                : ""; // eslint-disable-line
        if (tagId) {
    ...
    return function (md) {
        ...
        Object.keys(mapping).forEach(function (key) {
            md.renderer.rules[key] = function (tokens, idx) {
                return tokens[idx].type === "reference_note"
